...
contract GatewayMock {
    ...
    function nothing(string memory _simple) external pure returns(string memory message)  {
        message = _simple;
    }

    function test1(SimpleStruct1 memory _simple) external pure returns(string memory message)  {
        _simple = _simple;
        message = "holaa";
    }

    function test2(SimpleStruct2 memory _simple) external pure returns(string memory message)  {
        _simple = _simple;
        message = "holaa";
    }

    function getMessage(Voucher memory _voucher) external pure returns(string memory message)  {
